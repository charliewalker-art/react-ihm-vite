...
export const CarteCommandes = ({ commandes, loading }: CarteCommandesProps) => (
  <div className="px-4 py-4 flex flex-col gap-3 max-w-2xl mx-auto w-full">
    <h2 className="font-bold text-white text-lg">Mes commandes</h2>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {commandes.map((commande) => (
        <div key={commande.id} className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
          ...
          <div className="px-4 py-3 flex flex-col gap-1.5">
            {commande.details.map((d, i) => (
              <div key={i} className="flex justify-between text-sm">
                <span className="text-gray-300">{d.quantite}× {d.platNom}</span>
                <span className="text-amber-400">{d.sousTotal.toLocaleString('fr-MG')} Ar</span>
              </div>
            ))}
